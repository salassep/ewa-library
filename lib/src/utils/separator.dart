...
class Separator {
  ...
  static int getSeparatorIndex(List<int> bytesData) {
    ...
    for (var i = 0; i < bytesData.length; i++) {
      ...
      if (bytesData.length < (i + separatorList.length)) break;
  ...
  static List<int> separateBytesData(List<int> bytesData, int? separatorIndex) => bytesData.sublist(0, separatorIndex ?? getSeparatorIndex(bytesData));
